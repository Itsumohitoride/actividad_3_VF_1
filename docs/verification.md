...
- [ ] `kubectl apply --dry-run=client -k k8s/argocd/` — manifiestos K8s validos
...
#### F21: ArgoCD + GitOps
- [ ] `k8s/argocd/namespace.yaml` — Namespace argocd
- [ ] `k8s/argocd/install.yaml` — Componentes minimos ArgoCD v2.10
- [ ] `k8s/argocd/project.yaml` — AppProject microservice
- [ ] `k8s/argocd/application.yaml` — Application con auto-sync, self-heal, prune
- [ ] `k8s/argocd/kustomization.yaml` — Kustomize overlay
- [ ] `kubectl apply --dry-run=client -k k8s/argocd/` — manifiestos K8s validos
- [ ] `helm lint ./charts/*` — charts referenciados por ArgoCD son validos
- [ ] ArgoCD instalado en cluster
- [ ] Repositorio Git configurado como fuente
- [ ] Application con auto-sync, self-heal, prune
- [ ] Health checks (Actuator) funcionando
- [ ] Documentacion de UI y CLI en docs/deployment.md
